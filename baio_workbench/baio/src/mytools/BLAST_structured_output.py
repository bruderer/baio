...
def BLAST_api_query_generator(question: str):
    ...
    retrieved_docs = BLAST_db.as_retriever().get_relevant_documents(
        question
    ...
    for attempt in range(max_attempts):
        ...
        if "Status=WAITING" in status_text:
            ...
            if "ThereAreHits=yes" in status_text:
                print(
                    "{request_data.question_uuid} results are ready, retrieving and saving..."
                )
...
def blast_tool(question: str):
    ...
    BLAST_file_name = fetch_and_save_blast_results(
