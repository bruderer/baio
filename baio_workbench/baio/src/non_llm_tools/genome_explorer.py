...
class GeneInfoExtractor:
    ...
    def __init__(self, db_path):
        ...
        self.version = '0.1'  # should be the genome version from which the gene comes from
...
class SequenceExtractor:
    ...
    Args:
        ...
        start (int, optional): The start position of the gene sequence PYTHON INDEXING. Defaults to None.
        end (int, optional): The end position of the gene sequence PYTHON INDEXING. Defaults to None.
    ...
    def get_gene_sequence(self):
        ...
        sequence = self.fasta_dict[self.seqid].seq[self.start - PYTHON_SLICE_OFFSET:self.end]
...
class gffutilsFeatureToSeqFeature:
    ...
    def transform_feature(self):
        ...
        Returns:
            Bio.SeqFeature.SeqFeature: The transformed feature.
        """
        return SeqFeature(FeatureLocation(self.feature.start, self.feature.end), 
...
class RawInfoUpDater:
    ...
    def run_me(self):
        if self.raw_info.gene.strand == '-':
            print('Processing reverse complement: updating and re-indexing features to gene centric and genome centric')
            self.updated_sequence = self.transform_to_reverse_complement()
            self.features_gene_centric = self.transform_features_to_reverse_complement_gene_centric()
            self.features_genome_centric = self.transform_features_to_reverse_complement_genome_centric()
        if self.raw_info.gene.strand == '+':
            print('Processing forward sequence: updating features to gene centric and genome centric')
    ...
    def transform_features_to_reverse_complement_gene_centric(self):
        ...
        for feature in self.raw_info.cds:
            ...
            reverse_complement_feature.end = self.raw_info.gene.end - feature.start + self.PYTHON_OFFSET
            reverse_complement_feature.strand = 1
            reverse_complement_feature.type = feature.featuretype  # Preserve the type attribute                        
            reverse_complement_feature = gffutilsFeatureToSeqFeature(reverse_complement_feature).seqfeature
    ...
    def transform_features_to_reverse_complement_genome_centric(self):
        ...
        for feature in self.raw_info.cds:
            ...
            reverse_complement_feature.end = self.total_contig_length - feature.start +self.PYTHON_OFFSET
            reverse_complement_feature.strand = 1
...
class Concatenater:
    ...
    def __init__(self, sequence, features):
        self.features =  deepcopy(sorted(features, key=lambda feature: feature.location.start))
        self.concatenated_sequence = Seq(deepcopy(''.join([str(feature.extract(sequence)) for feature in self.features])))
    ...
    def reindex_features(self):
        ...
        for feature in self.features:
            ...
            if cumulative_length == 0:
                reindexed_feature.location = FeatureLocation(cumulative_length, cumulative_length + feature_length, strand=feature.location.strand)
            else:
                reindexed_feature.location = FeatureLocation(cumulative_length, cumulative_length + feature_length, strand=feature.location.strand)
...
def make_genbank_file(id, name, description, sequence, features, molecule_type, output_dir, suffix=''):
...
class GenomeRegionExtractor:
    ...
    def region_extractor(self):
        ...
        """
        self.region_features = list(self.get_features_in_region(self.start+1, self.end))
    ...
    def adapt_region_futures(self):
        ...
        if self.strand == '-':
            self.region_sequence = self.fasta_dict[self.seqid].seq[self.start:self.end].reverse_complement()
            seq_features = []
            ...
            for feature in self.region_features:
                ...
                feature.end = new_end
                feature.strand = 1  # Set the strand attribute to -1 when the strand is '-'
        ...
        if self.strand == '+':
            self.region_sequence = self.fasta_dict[self.seqid].seq[self.start:self.end] 
            seq_features = []
...
class FlankingRegionConcatenator:
    ...
    def __init__(self,
        ...
        self.flanking_region_features = [f for f in flanking_region_features if f.type == 'CDS']
    ...
    def update_sequence_with_upstream_region(self):
        ...
        for feature in self.base_features:
            ...
            update_feature.location = FeatureLocation(feature.location.start + bp_upstream,
        ...
        self.flanking_and_base_features = self.flanking_region_features + self.updated_base_features
...
class GeneAnalyser:
    def __init__(self, db_path, fasta_path, gene, output_dir, up_stream_bp= None, down_stream_bp= None):
        """
    ...
    def run_me(self):
        ...
        self.raw_sequence = SequenceExtractor(self.db_path, self.fasta_path, self.gene.seqid, self.gene.start, self.gene.end)
        
        #update the raw info
        self.up_dated_info = RawInfoUpDater(self.total_contig_length, self.raw_sequence, self.raw_info)
        self.up_dated_info.run_me()
        if self.raw_info.gene.strand == '-':
            print('saving full sequence; strand is negative')

            #save the reverse complement sequence with introns
            make_genbank_file(self.gene.id, 
                              self.gene.id, 
        ...
        self.concatenated = Concatenater(self.up_dated_info.updated_sequence, self.up_dated_info.features_gene_centric)
        ...
        self.up_stream_flanking_region = GenomeRegionExtractor(self.db_path, self.fasta_path,
                                                                start, end, 
    ...
    def get_upstream_sequence_for_concatenated_sequence(self):
        ...
        self.concatenated_upstream_sequence = FlankingRegionConcatenator(self.concatenated.concatenated_sequence,
                                                                            self.concatenated.concatenated_features, 
    ...
    def get_upstream_sequence_for_updated_sequence(self):
        ...
        self.upstream_sequence = FlankingRegionConcatenator(self.up_dated_info.updated_sequence,
                                                                            self.up_dated_info.features_gene_centric, 
                                                                            self.up_stream_flanking_region.region_sequence,
                                                                            self.up_stream_flanking_region.region_features)            
...
def genome_explorer(identifies:list, genome_db_path, genome_fasta_path, output):
    for gene_identifier in identifies:
        ...
        for gene in gene_list.gene:
            analyzer = GeneAnalyser(genome_db_path, genome_fasta_path, gene, output_dir=f'./{output}/{gene_identifier}_{i}/', up_stream_bp=5000)
            # Run the analysis
            analyzer.run_me()
